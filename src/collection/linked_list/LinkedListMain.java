...
public class LinkedListMain {
	...
	public static void main(String[] args) {
		...
		/*
		 * ArrayList클래스와 LinkedList클래스의 차이점 1. ArrayList<E>의 특징, 배열의 특징과 일치한다
		 * 저장소의 용량을 늘리는 과정에서 많은 시간이 소요된다. ArrayList<E>의 단점 --> 많은 데이터의 추가 /
		 * 삭제시에는 배열의 복사기 빈번하게 일어나기 때문에 성능 저하 데이터의 삭제에 필요한 연산과정이 매우 길다
		 * ArrayList<E>의 단점 데이터의 참조가 용이해서 빠른 참조가 가능하다. ArrayList<E>의 장점 -->
		 * 각데이터에 대한 인덱스를 가지고 있기 때문에 검색이 빠르다.
		 * 
		 * 2. LinkedList<E>의 특징, 리스트 자료구조의 특징과 일치한다. 저장소의 용량을 늘리는 과정이 간단하다.
		 * LinkedList<E>의 장점 --> 데이터의 추가 / 삭제는 위치정보의 수정만으로 가능하기 때문에 많은 정보의 추가 /
		 * 삭제 처리가 필요필요할때 유용하다. 데이터의 삭제가 매우 간단하다. LinkedList<E>의 장점 데이터의 참조가 다소
		 * 불편하다. LinkedList<E>의 단점 --> 데이터가 많은 경우의 검색시 처음 자료부터 순차적으로 찾아 나가야
		 * 하기때문에 느려진다.
		 */
	}
